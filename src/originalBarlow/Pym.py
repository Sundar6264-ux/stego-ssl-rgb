...
def train_barlow_twins(model, loader, epochs=800, base_lr=0.2, wd=1e-4, device='cuda'):
    ...
    scaler    = GradScaler("cuda")   # unified API, positional device
    ...
    for epoch in range(1, epochs+1):
        ...
        for (x1, x2), _ in pbar:
            ...
            optimizer.zero_grad()
            with autocast("cuda"):         # unified API, positional device
                z1 = model(x1)
                z2 = model(x2)
...
if __name__ == '__main__':
    # smoke‑test with 1 epoch by uncommenting below:
   # train_barlow_twins(..., epochs=1); train_linear_probe(..., epochs=1); finetune(..., epochs=1)
    main()
  # if __name__ == '__main__':
    #device = 'cuda' if torch.cuda.is_available() else 'cpu'
    #print(f"Using device: {device}\n")

    # ─── SSL Smoke‑Test (1 epoch) ────────────────────────────────────────────
   # ssl_loader = get_ssl_dataloader(batch_size=256)
   # bt_model   = BarlowTwinsModel(backbone='resnet50',
     #                             proj_hidden_dim=512,
    #                              out_dim=512)
   # train_barlow_twins(bt_model, ssl_loader,
    ...
    # ─── Linear‑Probe Smoke‑Test (1 epoch) ──────────────────────────────────
   # feat_dim     = bt_model.projector[0].in_features
   # probe        = LinearProbe(bt_model.encoder, feat_dim, n_classes=10)
   # train_loader = get_supervised_loader(batch_size=256, train=True)
   # val_loader   = get_supervised_loader(batch_size=256, train=False)
  #  train_linear_probe(probe, train_loader, val_loader,
     #                  epochs=1,       # only 1 epoch
    #                   lr=0.1,
   #                    mixup_alpha=1.0,
  #                     label_smoothing=0.1,
 #                      device=device)
#
    # ─── Fine‑Tune Smoke‑Test (1 epoch) ────────────────────────────────────
   # head       = nn.Linear(feat_dim, 10).to(device)
   # full_model = nn.Sequential(bt_model.encoder,
      #                         nn.Flatten(),
     #                          head)
    #finetune(full_model, train_loader, val_loader,
    #         epochs=1,        # only 1 epoch
   #          lr=1e-3,
  #           wd=1e-4,
 #            device=device)

#    print("✅ Smoke test complete—everything ran for 1 epoch without crashing.")
