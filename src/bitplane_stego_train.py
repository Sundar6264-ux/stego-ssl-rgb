...
class BitPlaneStego:
    ...
    def __call__(self, img: Image.Image) -> Image.Image:
        ...
        # XOR the mask to flip that bit‐plane across all channels
        arr ^= self.mask
...
class StegoTwoCropsTransform:
    ...
    def __call__(self, x):
        view1 = self.base(x)
        # apply stego on the PIL image before base transform
...
def main():
    ...
    _ = train_linear_probe(probe, tr_loader, va_loader,
                            epochs=100, lr=0.1,
                            mixup_alpha=1.0,
                            label_smoothing=0.1,
                            device=device)
    ...
    _ = finetune(full_model, tr_loader, va_loader,
                 epochs=50, lr=1e-3, wd=1e-4, device=device)
